...
def nst(config):
    '''
    Should read content img, style img, and hyperparameters from config
    '''
    ...
    if config['init_method'] == 'content':
        ...
        optimizing_img = Variable(style_img.clone(), requires_grad=True)
    elif config['init_method'] == 'random':
        gaussian_noise = np.random.normal(0, 1, content_img.shape).astype('float32')
        white_noise_img = np.random.uniform(-90., 90., content_img.shape).astype(np.float32)
        optimizing_img = Variable(torch.from_numpy(gaussian_noise).to(device), requires_grad=True)

    # prepare model
    neural_net, content_fms_index_name, style_fms_indices_names = utils.prepare_model(config['model'], device)
    print("Content feature maps index name: ")
    print(content_fms_index_name)

    print("Style feature maps indices names: ")
    ...
    target_content_representation = content_img_set_of_feature_maps[content_fms_index_name[0]].squeeze(axis=0)
    target_style_representation = [utils.gram_matrix(x) for cnt, x in enumerate(style_img_set_of_feature_maps) if cnt in style_fms_indices_names[0]]
    ...
    if config['optimizer'] == 'lbfgs':
        ...
        def closure():
            ...
            if torch.is_grad_enabled():
                optimizer.zero_grad()
            total_loss, content_loss, style_loss, tv_loss = build_loss(neural_net, optimizing_img, target_representations, content_fms_index_name[0], style_fms_indices_names[0], config)
